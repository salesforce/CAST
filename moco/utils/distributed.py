...
def launch(
    ...
    dist_backend: str, optional (default = "nccl")
        Which backend to use for distributed launch, choices ``{"nccl", "gloo"}``.
    args: Tuple
    ...
    if world_size > 1:
        mp.spawn(
            ...
            args=(
                job_fn, world_size, num_gpus_per_machine, machine_rank,
                dist_url, dist_backend, args
            ),
            daemon=False,
        )
    else:
        # Default to single machine, single GPU, with ID 0.
        _job_worker(0, job_fn, 1, 1, 0, dist_url, dist_backend, args)
    # fmt: on
